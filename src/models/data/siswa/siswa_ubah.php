                            ...
                            $pageItemObject = array(
                                    ...
                                    "breadcrumb" => array(
                                            array(
                                                    ...
                                                    "title" => "Ubah Petugas",
                                                    "link"  => UtilsManager::generateRoute('siswa_ubah',
                                                            ['id' => $_GET['id']])
                                            ),
                                    )
                            );
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $nisn = UtilsManager::getPostQuery('nisn');
    $nis = UtilsManager::getPostQuery('nis');
    $nama = UtilsManager::getPostQuery('nama');
    $alamat = UtilsManager::getPostQuery('alamat');
    $no_telp = UtilsManager::getPostQuery('no_telp');
    $id_kelas = UtilsManager::getPostQuery('id_kelas');
    $id_spp = UtilsManager::getPostQuery('id_spp');

    $payload = new UpdateSiswaDataDefinition(
            $nama,
            $nisn,
            $result['password'],
            $nis,
    ...
    try {
        $set = "nisn = '$payload->nisn', nis = '$payload->nis', nama = '$payload->nama', id_kelas = '$payload->id_kelas', alamat = '$payload->alamat', no_telp = '$payload->no_telp', id_spp = '$payload->id_spp'";
